...
- **Validation mode** (`MIRIN_SMOKE_VALIDATE=1`): instead of the blind autoquit timer, `m1-smoke` watches the engine event queue for `core.ready` + `window.created` (GPU process enabled), then self-quits — exit 0 only when the UI actually booted, nonzero on timeout (`MIRIN_SMOKE_TIMEOUT_MS`, default 30 s). CI runs this on Linux under Xvfb (`GPU UI smoke` step in `ci.yml`).
