...
- Startup cleanup runs asynchronously after any replacement-readiness receipt. Install siblings carry process-session and creation ownership, reject current-PID reuse, and lease otherwise unverifiable live owner/helper PIDs for at most 24 hours. Non-current generation owners share that bounded lease from their directory modification time.
