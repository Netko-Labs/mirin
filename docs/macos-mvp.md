...
- `app.updater`: packaged-app check/download/apply flow with fixed-size `Resources/version.json` reads, Ed25519-signed manifests, bounded network deadlines, strictly newer SemVer selection, single-flight checks, process/session-owned generation staging, an 8 GiB streaming reconstructed-tar/decompression ceiling, a 512 MiB combined in-memory patch-input ceiling, SHA-256 verification, archive type/link validation, real `.app`/host/recovery-codec/identity checks, stable installed designated-requirement verification (codesign-valid fallback for authenticated ad-hoc local builds), and whole-`.app` swap + relaunch. Before spawning user code, the host acquires an exclusive native per-app file lock for updater-capable processes or a shared lock for multi-instance processes; automatic apply requires a protocol-compatible Worker and the exclusive capability. Before terminal handoff, the validated `.app` is copied, revalidated, and recursively flushed beside the install; the bundled codec validates the real operands and probes `renamex_np(RENAME_SWAP)` on their filesystem. The helper acknowledges its prerequisites only after the probe succeeds. Only its token-bearing target may cross the reservation, and it must acquire the exclusive lock. The helper durably journals each phase, atomically exchanges the complete apps, directly launches the target, and retains the unique old-`.app` backup until the exact creation-identified process durably reports Worker/native readiness. macOS uses creation-bound kqueue waiting but, because it has no unprivileged handle-bound signal operation, refuses unsafe forced termination and preserves the transaction when a process will not exit; confirmed exit permits atomic rollback before reopening the old app. After helper death or reboot, one bootstrap atomically claims the journal before loading the core/Worker and either rolls back a pre-commit target through an external helper or completes committed cleanup. Failure to confirm helper/replacement death or rollback preserves terminal ownership and both trees. Successful helpers remove their generation, while startup prunes abandoned generations and exact dead-owner install siblings without touching live app/helper work.
- Native terminal shutdown begins before synchronous updater completion listeners, and its latch plus auto-check shutdown is shared by every public updater instance. Before arming, the helper verifies an identity-bound activation published only after its PID and macOS start time are durably recorded in the reservation, so an unactivated helper cannot swap after a parent crash and PID reuse cannot authorize another process. Mirin's first internal `ready` listener writes any replacement-readiness receipt before user listeners and defers only asynchronous startup cleanup. Install siblings carry process-session and hashed creation ownership, reject current-PID reuse, and lease live owners only while their exact identity matches. Non-current generation owners share that exact identity plus bounded modification-time lease, and process-wide generation allocation keeps public updater instances disjoint.
- Install-sibling names use a bounded hash of the app basename and retain legacy full-name cleanup compatibility. If atomic exchange becomes visible before its parent sync fails, a distinct codec status lets apply and recovery retry durability and preserve both trees on continued failure.
