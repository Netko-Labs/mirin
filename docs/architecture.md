...
3. Resolve configuration and acquire the process-lifetime app lock before user
   code starts. Single-instance processes take it exclusively; multi-instance
   processes take it shared, so the two modes cannot overlap. Windows also keeps
   its named mutex for compatibility and existing-window activation, while
...
4. Spawn the user's main-process code as a **Bun Worker** (`new Worker(appEntry)`),
   passing whether the singleton was actually acquired.
...
PowerShell and records its actual PID, so the running app quits only after the complete
helper is accepted. That acceptance is the terminal handoff point: updater
...
- **Bundle + distribution.** No `.app`/codesign — a flat app folder like Windows:
  ...
  generations without touching live-process work. Packaging (AppImage / `.deb` / `.rpm`)
  is landing this release — see
