...
the reservation. Arming and replacement readiness are each published by atomic
rename, so readers never reject a partially written PID. If arming fails after
activation, the parent performs bounded terminate/kill/wait and abandons the
