...
3. Resolve configuration, honor any active updater handoff reservation, and
   ...
   scoped named mutex for compatibility and existing-window activation, while
...
folder swap keeps its backup until the replacement writes a durable readiness receipt;
failure or timeout rolls back and reopens the prior install. Consumers
