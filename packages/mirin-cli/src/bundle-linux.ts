...
import { cpSync, mkdirSync, rmSync, writeFileSync, existsSync, readdirSync, chmodSync, statSync } from "node:fs";
...
const CEF_RUNTIME_EXTRAS = ["locales", "vk_swiftshader_icd.json", "libvulkan.so.1", "chrome-sandbox"];
  ...
  if (statSync(icon).isDirectory()) {
    ...
    const pick =
      ...
      pngs
        ...
        .find((p) => iconsetPixels(p) <= MAX_SIDE * MAX_SIDE) ??
...
export function desktopEntry(opts: {
  ...
  if (opts.categories) {
    lines.push(`Categories=${opts.categories.endsWith(";") ? opts.categories : `${opts.categories};`}`);
  }
  lines.push("");
  return lines.join("\n");
}
...
function copyCefRuntime(cefPath: string, dest: string): void {
