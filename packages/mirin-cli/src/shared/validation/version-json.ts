...
import { validateUpdatePublicKey } from "./update-key.ts";
...
/** Serialize the six-field updater metadata envelope and verify its readback. */
export function serializeVersionMetadata(input: VersionMetadataInput): string {
  ...
  const metadata: VersionMetadata = {
    ...
    publicKey: validateUpdatePublicKey(input.publicKey),
...
export function parseVersionMetadata(serialized: string): VersionMetadata {
  ...
  if (
    ...
    throw new Error("[mirin] invalid version.json — expected exactly six identity fields.");
  }
  ...
  return {
    ...
    publicKey: validateUpdatePublicKey(object.publicKey),
    name: identity.appName,
    identifier: identity.bundleId,
  };
}
