...
export function renderPosixRecovery(
  ...
  return [
    ...
    "done",
    "armed_status=0",
    '"$TOOL" durable-write "$ARMED" "$SELF_IDENTITY" || armed_status=$?',
    'if [ "$armed_status" -ne 0 ]; then',
    "  # A matching visible receipt already transferred recovery ownership.",
    '  ARMED_IDENTITY=""; if [ -f "$ARMED" ]; then IFS= read -r ARMED_IDENTITY < "$ARMED" || true; fi',
    '  test "$ARMED_IDENTITY" = "$SELF_IDENTITY"',
    '  "$TOOL" sync-parent "$ARMED" 2>/dev/null || true',
    "fi",
...
export function renderWindowsRecovery(
  ...
  return [
    ...
    `  if (Test-Path -LiteralPath ${psq(armed)} -PathType Leaf) {`,
    `    $visibleArmedIdentity=(Get-Content -LiteralPath ${psq(armed)} -Raw).Trim()`,
    "  }",
    "  if ($visibleArmedIdentity -ne $selfIdentity) { throw 'Could not publish updater recovery acceptance' }",
    `  & $tool sync-parent ${psq(armed)}`,
    "}",
